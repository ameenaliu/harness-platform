...
- All branches follow the single-branch model: ONE user branch cut off `develop` (the Integration Branch) — `users/<user-slug>/<impl-slug>` (Story) or `users/<user-slug>/bugs/<impl-slug>` (Bug). **PR base = `develop` for everything.** There is NO `features/<feature-slug>/main` branch, ever. The item **Feature** (Epic → Feature → Story → Task) is a **backlog grouping only** — identified for sub-issue linking, board Parent grouping, and PR-body context, but it does **not** affect git branches or PR base ("Item-Feature ≠ git branch"). Branch names are parameterized in `platform-context.md` (defaults `main`/`develop`). See `commands/plan.md`.
