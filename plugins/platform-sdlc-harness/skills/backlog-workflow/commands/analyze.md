...
| **Parent Feature** | Linked as a sub-issue of a Feature (or `Parent: #<n>` in the body) — mandatory unless `type:bug` | 🔴 Critical for non-Bugs — drives dev-workflow branch routing (`features/<feature-slug>/main` vs `develop`); suggest checking with `/discovery-workflow extend` if the Feature exists |
