...
- [ ] Working branch matches `^users/[a-z0-9_]+/(features|bugs)/[a-z0-9-]+$` (stories `users/<slug>/features/<impl>`, bugs `users/<slug>/bugs/<impl>`) and the PR base is `develop` (single-branch model — no feature branch)
