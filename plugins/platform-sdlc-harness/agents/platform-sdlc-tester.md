...
Then resolve the affected Surface(s) to their **stack pack**: read each surface's stack from `.claude/context/platform-context.md` (Surfaces & Stack table), look it up in `packs/registry.json` → `packs/<stack>/pack.json`, and load that pack's `conventions_skill`. Use the pack's `test_frameworks`, `commands.test` (and `commands.e2e` where present), and `coverage_threshold` rather than assuming a stack. Those sources are authoritative; this file is only a pointer.
