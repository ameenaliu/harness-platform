...
- **Git hygiene**: working branch matches `^users/[a-z0-9_]+/(bugs/)?[a-z0-9-]+$` (stories `users/<slug>/<impl>`, bugs `users/<slug>/bugs/<impl>`); PR base is `develop` for every Story and Bug (single-branch model — no feature branch); no merge commits from base onto the user branch (rebase instead); no build-breaking commits.
