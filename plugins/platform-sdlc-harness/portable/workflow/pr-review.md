...
7. **Evaluate the whole change set holistically** — not per task: cohesion across all commits, no leftover scaffolding, consistent naming, no cross-commit duplication, the linked Story's acceptance criteria met end-to-end, security, and git hygiene (branch naming matches `^users/[a-z0-9_]+/(bugs|[a-z0-9-]+)/[a-z0-9-]+$`, Conventional Commits with surface scope, no issue IDs in commits, `Closes #<story>` present in the PR body, no build-breaking commits).
